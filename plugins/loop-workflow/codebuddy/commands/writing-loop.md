---
description: Writing-Loop: 文档写作质量信号闭环（写作边界）
...
状态文件路径：`.loop-cli/state/writing-loop.json`
...
### 写入规则
- 每轮结束时写入，先写 `.loop-cli/state/writing-loop.json.tmp`，再重命名为 `.loop-cli/state/writing-loop.json`（防止写入中断导致文件损坏）。
...
5. **复杂度评估与再拆分**：按 Writing Orchestrator agent 模板「## 任务复杂度评估与拆分」的判据（accept_criteria > 3 / 跨文档文件 > 5 或跨文档模块 > 2）逐项评估，对过大任务原地拆成子任务，`subtask_of` 标注溯源，新 `depends_on` 接入拓扑。**拆分不得越界**——每个子任务 `target_docs` 必须仍在 hard_scope 内。核算子任务总数 ≤ 4（`= MAX_CYCLES × 0.67`），超限且无法再拆时输出 `action="HOLD"`、`reason="decomposition_overflow"` 交用户。
6. **加载术语表**：从 `.opencode/terms.json` 或 `docs/glossary.md` 读取 `[term, preferred_form, definition]`。
7. 探测项目脚本（lint，如 markdownlint / vale）。
8. 初始化 `consecutive_failures = 0`、`stall_counter = 0`、`round = 0`、`stop_reason = null`。
9. 设置 `MAX_CYCLES = 6`（超过此轮次仍未 DONE 则强制停止，writing 容错性高）与 `STALL_MAX = 2`（连续 2 轮任务状态签名无变化则判 STALL）。二者均为初始化硬上限，不被 `fail_history` 或 `round` 覆盖。每轮约消耗 2-3 个 agentic step，确保 `steps >= MAX_CYCLES × 3`。
