...
单会话步数预算是硬约束（orchestrator frontmatter `steps: 60`，每轮约 2-3 步，故 `max_iterations` 覆盖硬上限为 20 轮）。任务规模超出预算时，闷头执行必然撞 `MAX_CYCLES` 硬终止。初始化阶段必须先评估每个任务的复杂度，对超标任务原地拆成子任务，让总规模落入预算。
