...
// 当前发布仅含 coding + ralph 两个域；如需新增 testing/writing 域，请同时恢复 templates 与对应 MAP 条目
const AGENT_MAP = {
...
function processAgents() {
  ...
  if (!fs.existsSync(srcDir)) {
    console.error(`Template dir missing: ${srcDir}`);
    console.error('Add new loop-workflow templates locally and re-run, or restore templates/ via git history.');
    process.exit(1);
  }
  ...
  for (const file of files) {
    ...
    if (!map) { console.error(`MISSING MAP: ${file} (add to AGENT_MAP or remove this template)`); process.exit(1); }
...
function processCommands() {
  ...
  for (const file of files) {
    ...
    if (!map) { console.error(`MISSING MAP: ${file} (add to COMMAND_MAP or remove this template)`); process.exit(1); }
    const content = fs.readFileSync(path.join(srcDir, file), 'utf8');
    const replaced = replacePlaceholders(content, map);
    for (const platform of PLATFORMS) {
