---
description: Ralph-Graph: 通用编排执行审查闭环（DAG 路由表）
...
1. **状态注入**：检查状态文件 `.loop-cli/state/ralph-graph.json` 是否已包含 `nodes` 映射。若由外部工具注入且有效（含拓扑字段） $\rightarrow$ 直接使用。
...
状态文件路径：`.loop-cli/state/ralph-graph.json`
...
```json
{
  ...
  "nodes": {
    "<节点 id>": {
      ...
      "failures": 0,
      "result": null,
      "subtask_of": "<被拆分来源节点 id，仅子节点填写，用于追溯>"
    }
  },
...
### 写入规则
- 每轮结束时写入，先写 `.loop-cli/state/ralph-graph.json.tmp`，再重命名为 `.loop-cli/state/ralph-graph.json`（防止写入中断导致文件损坏）。
