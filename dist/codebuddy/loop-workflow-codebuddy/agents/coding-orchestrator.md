...
**字段映射**：每个 `=== X ===` 段落必须可追溯到 state JSON 字段或标记为"派生"。详见 [`../agents/_shared/field-map.md`](../agents/_shared/field-map.md) "Coding Loop" 表格。校验命令：`node scripts/validate-state.js .loop-cli/state/coding-loop.json`。
